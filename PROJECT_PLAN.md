...
- Owners: `ui-shell` (PWA/config), `persistence-economy` (data stubs).
...
- Local leaderboard in IndexedDB.
- **DoD:** installs and runs offline on desktop + mobile; leaderboard persists across sessions.
- Owners: `ui-shell`, `persistence-economy`.
...
- Coins (points → coins baseline), unlock/purchase system, shop UI, persistent unlocks.
- **DoD:** earn coins in classic mode, buy a skin/theme, unlock persists; economy numbers in config; unlocks cosmetic only.
- Owners: `persistence-economy`, `ui-shell`.
...
- Remote backend behind the existing persistence adapter (e.g. Supabase).
- Minimal server-side score validation; note anti-cheat limits (client scores are spoofable).
- **DoD:** global scores submit and display; local still works offline; no game/UI changes beyond the adapter.
- Owners: `persistence-economy`.
